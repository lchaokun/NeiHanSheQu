...
public class JDMediaScannerConnectionClient implements MediaScannerConnection
    ...
    public void onScanCompleted(String path, Uri uri) {
        Looper.prepare();
        if (isSmallPic) {
            ToastUtils.showLong(MyApplication.getContext(),ConstantString.SAVE_SMALL_SUCCESS + " \n相册" + File.separator + CacheUtil
                    .FILE_SAVE + File.separator + newFile.getName());
        } else {
            ToastUtils.showLong(MyApplication.getContext(),ConstantString.SAVE_SUCCESS + " \n相册" + File.separator + CacheUtil
                    .FILE_SAVE + File.separator + newFile.getName());
        }
        Looper.loop();
    }
